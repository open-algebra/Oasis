...
auto Exponent<Expression>::Integrate(const Expression& integrationVariable) const -> std::unique_ptr<Expression>
{
    ...
    if (auto variable = RecursiveCast<Variable>(integrationVariable); variable != nullptr) {
        ...
        if (auto realExponent = RecursiveCast<Exponent<Variable, Real>>(*simplifiedExponent); realExponent !=
            nullptr) {
    ...
    const Expression& differentiationVariable) const -> std::unique_ptr<Expression>
{
    ...
    if (auto variable = RecursiveCast<Variable>(differentiationVariable); variable != nullptr) {
        ...
        if (auto natBase = RecursiveCast<Exponent<EulerNumber, Expression>>(*simplifiedExponent); natBase !=
            nullptr) {
        ...
        if (auto generalCase = RecursiveCast<Exponent<Expression, Expression>>(*simplifiedExponent); generalCase !=
            nullptr) {
            Multiply derivative{
                Multiply{
                    *simplifiedExponent,
